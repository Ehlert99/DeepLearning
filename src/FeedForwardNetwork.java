...
public class FeedForwardNetwork {
  ...
  public FeedForwardNetwork(int numberOfNetworkInputSignals, int numberOfNeuronsInHiddenLayer, int numberOfNeuronsInOutputLayer) {
    ...
    if (TrainingParameter.weightsOfHiddenLayer != null)
      setWeightsOfHiddenLayer();  // in the case of given weights
    if (TrainingParameter.weightsOfOutputLayer != null)
      setWeightsOfOutputLayer();  // in the case of given weights
  }

  private void builtHiddenLayer(int numberOfLayerInputSignals, int numberOfNeuronsInHiddenLayer) {
  ...
  private void builtOutputLayer(int numberOfLayerInputSignals, int numberOfNeuronsInOutputLayer) {
  ...
  public void setWeightsOfHiddenLayer() {
    for (int i = 0; i < hiddenLayer.size(); i++)         // number of outputs (hidden layer)
      for (int j = 0; j < inputLayer.length + 1; j++)    // number of inputs  (hidden layer) + 1 (bias)
        hiddenLayer.get(i).setWeights(j, TrainingParameter.weightsOfHiddenLayer[i][j]);
  }

  public double[][] getWeightsOfHiddenLayer() {
    double[][] weightsOfHiddenLayer = new double[hiddenLayer.size()][hiddenLayer.size() + 1];  // + 1: bias!
    for (int i = 0; i < hiddenLayer.size(); i++)         // number of outputs (hidden layer)
      for (int j = 0; j < inputLayer.length + 1; j++)    // number of inputs  (hidden layer) incl. bias!
        weightsOfHiddenLayer[i][j] = hiddenLayer.get(i).getWeights()[j];
    return weightsOfHiddenLayer;
  }

  public void setWeightsOfOutputLayer() {
    for (int i = 0; i < outputLayer.size(); i++)         // number of outputs (output layer)
      for (int j = 0; j < hiddenLayer.size() + 1; j++)   // number of inputs  (output layer) + 1 (bias)
        outputLayer.get(i).setWeights(j, TrainingParameter.weightsOfOutputLayer[i][j]);
  }

  public double[][] getWeightsOfOutputLayer() {
    double[][] weightsOfOutputLayer = new double[outputLayer.size()][hiddenLayer.size() + 1];   // + 1: bias!
    for (int i = 0; i < outputLayer.size(); i++)         // number of outputs (output layer)
      for (int j = 0; j < hiddenLayer.size() + 1; j++)   // number of inputs  (output layer) incl. bias!
  ...
  private void calculateOutputFromHiddenLayer(double[] inputOfInputLayer) {
    for (int i = 0; i < hiddenLayer.size(); i++)
      ProcessMonitoring.lastOutputsFromHiddenLayer[i] = hiddenLayer.get(i).getOutput(inputOfInputLayer, TrainingParameter.activationFunction);
  }

  private void calculateOutputFromOutputLayer() {
    for (int i = 0; i < outputLayer.size(); i++)
      ProcessMonitoring.lastOutputs[i] = outputLayer.get(i).getOutput(ProcessMonitoring.lastOutputsFromHiddenLayer, TrainingParameter.activationFunction);
  }
  ...
  public void trainWithSupervisedLearning() {
    for (int epoche = 1; epoche <= TrainingParameter.numberOfEpochs; epoche++) {
      ...
      System.out.println("\nepoche: " + epoche + "   number of data: " + (sample + 1));  // for testing
      calculateOutput(TrainingParameter.inputs[sample]);
      DisplayMachineLearning.showOutput(ProcessMonitoring.lastOutputs);                  // for testing
      double totalError = calculateTotalError(TrainingParameter.targets[sample]);
      System.out.println("\ntotal error before: " + totalError);                         // for testing
      makeBackPropagationForOutputLayer(TrainingParameter.targets[sample]);
      makeBackPropagationForHiddenLayer(TrainingParameter.inputs[sample], TrainingParameter.targets[sample]);
      calculateOutput(TrainingParameter.inputs[sample]);
      totalError = calculateTotalError(TrainingParameter.targets[sample]);
      System.out.println("\ntotal error after backpropagation: " + totalError);          // for testing
    }
  }
  ...
  private void makeBackPropagationForOutputLayer(double[] targets) {
    ...
    for (int i = 0; i < ProcessMonitoring.lastOutputs.length; i++) {
      ...
      for (int j = 0; j < numberOfWeightsInclBias; j++) {
        double singleWeightError = (targets[i] - lastOutputI) * (lastOutputI) * (1 - lastOutputI);
        double deltaW = singleWeightError * TrainingParameter.learningRate;
        deltaW *= (isNotABiasWeight(j, numberOfWeightsInclBias)) ? ProcessMonitoring.lastOutputsFromHiddenLayer[j] : 1;   // weight : bias
  ...
  public void testAllInputsAndShowResults() {
    ...
    for (int i = 0; i < TrainingParameter.inputs.length; i++) {
      ...
      for (int j = 0; j < TrainingParameter.targets[0].length; j++) {
        ...
        if (Math.abs(ProcessMonitoring.lastOutputs[j] - TrainingParameter.targets[i][j]) > TrainingParameter.faultTolerance)
  ...
  private double calculateDeltaW(int i, int j, double[] trainingData, double[] targets) {
    ...
    for (int k = 0; k < ProcessMonitoring.lastOutputs.length; k++) {
      ...
      errors[k] = (targets[k] - lastOutputK) * lastOutputK * (1 - lastOutputK) * getWeightsOfOutputLayer()[k][i];
      errorSum += errors[k];
    }
    int numberOfWeights = hiddenLayer.get(i).getWeights().length - 1;
    double inputJ = (j < numberOfWeights) ? trainingData[j] : 1;
    double deltaW = errorSum * ProcessMonitoring.lastOutputsFromHiddenLayer[i] * (1 - ProcessMonitoring.lastOutputsFromHiddenLayer[i]) * inputJ * TrainingParameter.learningRate;
    return deltaW;
  }
